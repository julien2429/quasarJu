...
function dataDownloadLink(element, text) {
  var linkText =
    "<a class='dataDownload' href='#' data-tag='" + element.tag + "'";
      ...
      try {
        ...
        function extractEncapsulatedImageFrame(frameIndex) {
          ...
          if (isRLE) {
            ...
            imageFrame = dicomParser.readEncapsulatedPixelDataFromFragments(
              dataSet,
              element,
              frameIndex,
            );
            ...
            imageFrame = dicomParser.readEncapsulatedImageFrame(
              dataSet,
              element,
              frameIndex,
              bot,
        ...
        $(".imageFrameDownload").click(function (ev) {
          var frameIndex = parseInt(
            $(ev.currentTarget).attr("data-frameIndex"),
          );
        ...
        $(".dataDownload").click(function (ev) {
          ...
          var dataOffset = parseInt(
            $(ev.currentTarget).attr("data-dataOffset"),
          );
          var length = parseInt($(ev.currentTarget).attr("data-length"));
          var data = dicomParser.sharedCopy(
            dataSet.byteArray,
            dataOffset,
            length,
          );
